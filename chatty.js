$(function () {
    ...
    // thanks electroly!
    var WinChattyThread = function _winChattyThread(args) {
        this.args = args;
        this.successCb = null;
        if (WinChattyThread.instance === null) {
            this.load();
        }
        else {
            return WinChattyThread.instance;
        }
    };

    WinChattyThread.prototype.load = function _load(cb) {
        this.postArgs = {};
        this.successCb = cb;
        for (var i = 0; i < this.args.length; i++) {
            this.postArgs['arg' + i] = this.args[i];
        }
        $.post('http://winchatty.com/service/json', this.postArgs, this.Threadsuccess, 'json').fail(this.failure);
    };
    WinChattyThread.prototype.instance = null;
    WinChattyThread.prototype.Threadsuccess = function _winchattyThreadSuccess(tree) {
        // http://programmers.stackexchange.com/questions/214227/
        var stack = [],
            replies = [],
            root = new Reply();

        if (tree.hasOwnProperty('faultString')) {
            return console.error('Server fault occurred: ' + tree.faultString);
        }
    ...
    WinChattyThread.prototype.failure = function _winchattyFailure(jqXHR, statusText, errorThrown) {
        console.error('POST failed due to: ' + ['Error:', ['[', jqXHR.status, ']: ', statusText].join(' '), 'Response:', errorThrown].join(' '));
    };

    var WinChattyStories = function _winChattyStories(data) {
        this.title = data[0].title;
        this.story_id = data[0].story_id;
    };

    WinChattyStories.prototype.load = function () {
    ...
    WinChattyStories.prototype.failure = function (error) {
        window.alert("Failed to access winchatty database: " + error);
    }

    var winChattyThread  = new WinChattyThread(["ChattyService.getThreadTree"]),
        winChattyStories = new WinChattyStories(["ChattyService.getStories"]);
